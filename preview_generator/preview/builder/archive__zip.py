...
def archive_info_to_text(archive_info):
    text = ''
    text__files = ''
...
def archive_info_to_html(archive_info):
    html = ''
    html__files = ''
    for property, value in archive_info.to_dict().items():
        if property == 'files':
...
class ZipPreviewBuilder(OnePagePreviewBuilder):
    ...
    def build_json_preview(self, file_path, preview_name,
    ...
    def zipfile_to_infos(self, zipfile):
